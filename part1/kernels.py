...
# @nki.compiler.skip_middle_end_transformations
# @nki.jit
# def matrix_transpose_slow(a_tensor):
#     M, N = a_tensor.shape
...
def matrix_transpose(a_tensor):
    ...
    for i in nl.affine_range(num_tiles_m):
        a_row_tile = nl.ndarray((nl.par_dim(tile_dim), N), dtype=a_tensor.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            src=a_tensor[i * tile_dim : (i + 1) * tile_dim, :],
            dst=a_row_tile
        )

        for j in nl.affine_range(num_tiles_n):
            input_tile = a_row_tile[:, j * tile_dim : (j + 1) * tile_dim]
            res_psum = nisa.nc_transpose(input_tile)
            output_tile = nisa.tensor_copy(res_psum, dtype=a_tensor.dtype)
            nisa.dma_copy(src=output_tile, dst=out[j * tile_dim : (j + 1) * tile_dim, i * tile_dim : (i + 1) * tile_dim])
